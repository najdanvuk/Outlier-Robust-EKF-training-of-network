function mlpnet = gen_MLP(lengths , numhidden,weightVar, biasVar)
...
if numel(numhidden) == 1
    ...
    mlpnet(1).biases = sqrt(biasVar) .* randn(numhidden,1);
    ...
    mlpnet(1).biases = sqrt(biasVar) .* randn(numhidden(1),1);
    mlpnet(2).biases = sqrt(biasVar) .* randn(numhidden(1),1);
end
